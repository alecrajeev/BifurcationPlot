...
5. Used the python program averagePeaks.py to bin the peaks and find average of each bin
...
experimental_data -------------- coordinates of the averaged peaks

experimental_data/full_data.csv  the full csv file used to form the bifucation diagram

formattedData2/findPeaks.py ---- python program that finds peaks using quadratic regression

maximumData2/averagePeaks.py --- python program that bins the peaks and finds the average of each bin

RLC_data2/xlsxTOcsv.py --------- python program from internet that converts xlsx to csv

RLC_data2/processCSV.py -------- python program that deletes first couple of unneccesary lines
...
plotTheoryData.js -------------- javascript file that plots theoretical data using D3 (data visualization library)
warning: takes long to load

plotExperimentailData.js ------- javascript file that uses D3 to 
...
The method described above for finding the local max values does not work well for the lower values that are peaks in a bifurcation plot. In fact, for much the data only the upper peaks are found. This is because for the lower peaks, the difference between the peak and the base is too small for the program to distinguish it from noise. This is an unfortunate side effect of using a program to find the peaks. Only later as the lower peak gets larger can you see it again.

Once the peak values are found more analysis is done. There are multiple peak values that represent the same spot in the bifurcation plot. So each of the peaks is put into its corresponding bin. Then the average value of that bin is calculated. This will be the y value for that point. Before bifurcation there are 0 bins. After the first bifurcation there are 2 bins. After the second bifurcation there are theoretically 4 bins, but experimentally you only see 3.
